...
const { findEmail, create, updateResetToken, findByResetToken, updatePassword, uploadAvatar: updateAvatar } = require("../models/UserModel");
...
const UserController = {
  ...
  getProfile: async (req, res, next) => {
    try {
      const {
        rows: [user],
      } = await findEmail(req.user.email);
  ...
  uploadAvatar: async (req, res) => {
    try {
      ...
      if (!userId) {
        return commonHelper.unauthorized(res, "User not authenticated");
      }
      ...
      // CloudinaryStorage sudah upload file, ambil URL dari req.file.path
  ...
  deleteAvatar: async (req, res, next) => {
    try {
      ...
      const {
        rows: [user],
      } = await pool.query("SELECT avatar_url FROM users WHERE id = $1", [userId]);
